...
Page({
  data: {
    ...
    btnconfirm:
      "https://dcdn.it120.cc/2024/04/12/21578e4c-575a-48dd-9825-072d92f7a3e8.png",
    clickLuck: "clickLuck",
    luckPosition: 0,
  },
  onLoad(e) {
  ...
  async luckyInfo() {
    ...
    if (res.code == 0) {
      ...
      let luckyGoods = res.data.map((ele) => {
        let data = {
          goodsId: ele.id, // 补全
          pic: ele.picUrl,
          title: ele.title,
  ...
  loadAnimation() {
    ...
    interval = setInterval(function () {
      ...
      e.data.border[index] =
        "border:6rpx solid #9077c4;border-radius:40rpx;width:158rpx;height:158rpx";
      e.setData({
  ...
  async getLuckyInfoJoin() {
    ...
    // const luckPosition = this.data.luckyGoods.findIndex(
    //   (ele) => ele.title == prizeName
    // ); // 服务器返回的中奖的是哪个商品
    // this.setData({
    //   luckPosition,
    // });
  ...
  clickLuck() {
    ...
    this.setData({
      btnconfirm:
        "https://dcdn.it120.cc/2024/04/12/816d31c4-cb18-4107-8934-00999356a682.png",
      clickLuck: "",
    });
    //清空计时器
  ...
  stopLuck(which, index, time, splittime) {
    ...
    setTimeout(function () {
      ...
      //当前位置为选中状态
      e.data.border[index] =
        "border:6rpx solid #ee6916;border-radius:40rpx;width:168rpx;height:168rpx";
      e.setData({
        border: e.data.border,
      });
      ...
      if (time < 400 || index != which) {
        ...
        setTimeout(function () {
          wx.showModal({
            title: "中奖提示",
            content:
              e.data.luckPosition == 6
                ? "非常遗憾，您没有中奖，谢谢您的参与。"
                : "恭喜抽中: " + e.data.luckyGoods[e.data.luckPosition].title,
            showCancel: false,
